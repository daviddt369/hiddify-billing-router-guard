...
class UserView(FlaskView):
    ...
    @route("/singbox-sandbox/")
    @route("/singbox-sandbox")
    @login_required(roles={Role.user})
    def singbox_sandbox(self):
        '''Returns a sandbox-safe singbox JSON config for proxy-status checks'''
        return self.full_singbox_imp(sandbox_mode=True)
    ...
    @login_required(roles={Role.user})
    def full_singbox_imp(self, sandbox_mode=False):
        ...
        if request.method == 'HEAD':
            ...
            resp = hutils.proxy.singbox.configs_as_json(**c, sandbox_mode=sandbox_mode)
