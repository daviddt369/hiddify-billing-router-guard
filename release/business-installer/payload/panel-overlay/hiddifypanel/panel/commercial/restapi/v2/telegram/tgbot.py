...
class ExceptionHandler(telebot.ExceptionHandler):
    def handle(self, exception):
        ...
        try:
            ...
            logger.error(f"Error during recovery attempt: {inner_exc}")
        return False


bot = prepare_telebot(
    telebot.TeleBot("1:2", parse_mode="HTML", threaded=False, exception_handler=ExceptionHandler())
)
bot.username = ""
...
def _webhook_domain_override() -> str:
    domain = (
        ...
        or os.environ.get("HIDDIFY_TELEGRAM_WEBHOOK_DOMAIN", "")
...
def _webhook_secret_is_valid(req) -> bool:
    ...
    def post(self):
        try:
            ...
            if not content_type.startswith("application/json"):
                logger.error("Telegram webhook rejected: invalid content-type=%s", content_type)
                return Response("", status=403)
            ...
            logger.exception("Telegram webhook request validation failed: %s", exc)
            return Response("", status=200)
