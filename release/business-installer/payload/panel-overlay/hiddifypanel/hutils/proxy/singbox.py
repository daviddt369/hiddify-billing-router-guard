...
def _replace_string_refs(value, old: str, new: str):
    if isinstance(value, dict):
        for key, item in value.items():
            if item == old:
                value[key] = new
            else:
                _replace_string_refs(item, old, new)
...
def configs_as_json(domains: list[Domain], **kwargs) -> str:
    ...
    sandbox_mode = bool(kwargs.get("sandbox_mode"))
    ...
    if sandbox_mode:
        # Sandbox / proxy-status should avoid selector/urltest/balancer completely
        # because older hiddify-cli builds convert these objects to unsupported
        # internal load-balance strategies.
        default_tag = select["outbounds"][0] if select["outbounds"] else "direct"
        _replace_string_refs(base_config, "Select", default_tag)
    else:
