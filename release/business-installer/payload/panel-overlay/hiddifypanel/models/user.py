...
class User(BaseAccount):
    ...
    def remove(self, commit=True) -> None:
        ...
        if "anti_share_event" in tables:
            from hiddifypanel.antishare.models import AntiShareEvent
            AntiShareEvent.query.filter(AntiShareEvent.user_id == self.id).delete(synchronize_session=False)
        if "anti_share_ip_profile" in tables:
            from hiddifypanel.antishare.models import AntiShareIPProfile
            AntiShareIPProfile.query.filter(AntiShareIPProfile.user_id == self.id).delete(synchronize_session=False)
        if "anti_share_state" in tables:
            from hiddifypanel.antishare.models import AntiShareState
            AntiShareState.query.filter(AntiShareState.user_id == self.id).delete(synchronize_session=False)
        if "anti_share_user_override" in tables:
            from hiddifypanel.antishare.models import AntiShareUserOverride
            AntiShareUserOverride.query.filter(AntiShareUserOverride.user_id == self.id).delete(synchronize_session=False)
